...
export default function OrderDetails() {
  ...
  const updateStatus = async (status) => {
    ...
    try {
      ...
      if (status === "accepted" && prepTime) payload.prepTime = prepTime;
      if (status === "rejected" && rejectionReason) payload.rejectionReason = rejectionReason;
      await updateOrderStatus(id, payload);
      toast.success(`Order ${status} successfully`);
      setTimeout(() => navigate("/restaurant/orders"), 1000);
    } catch (err) {
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <Button
                ...
                disabled={!prepTime}
                className={`w-full rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-300 ${prepTime
